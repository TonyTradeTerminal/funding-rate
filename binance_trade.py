...
# ---------------------------------------------------------------------------
# One-shot queries (全部放到循环外)
# ---------------------------------------------------------------------------

now_ms = int(time.time() * 1000)

premium_map = {p["symbol"]: p for p in client.futures_mark_price()}          # markPrice + lastFundingRate
funding_map = {f["symbol"]: f for f in client.futures_funding_rate(limit=1)} # 最近一次资金费率
adl_map = {d["symbol"]: d["adlQuantile"] for d in client.papi_get_um_adl_quantile(timestamp=now_ms)}

account = client.papi_get_um_account(timestamp=now_ms)

# ---------------------------------------------
# Build dataframe (循环里不再直接调用 client 方法)
# ---------------------------------------------
...
for pos in account["positions"]:
    ...
    fr_nxt_raw = float(prem["lastFundingRate"])
    fr_nxt_ts = pd.to_datetime(int(prem["nextFundingTime"]), unit="ms")
    int_nxt_8h = float(prem["interestRate"]) * 100

    fund_row = funding_map.get(symbol, {})
    fr_last_raw = float(fund_row.get("fundingRate", fr_nxt_raw))
    fr_last_ts = pd.to_datetime(
        int(fund_row.get("fundingTime", fr_nxt_ts.value // 10 ** 6)), unit="ms"
    )

    # ADL quantile（已预取）
    adl_q = adl_map.get(symbol, {})
    adl_ranking = adl_q.get("LONG") or adl_q.get("BOTH") or adl_q.get("SHORT")

    # Recent PnL & fees
    r_pnl, _ = last_income(symbol, "REALIZED_PNL")
    pnl_fund, _ = last_income(symbol, "FUNDING_FEE")
...
futures_df = pd.DataFrame(futures_data).sort_values("asset")
