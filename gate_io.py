...
class Gateigate_io:
    def __init__(self, symbol):
        self.symbol = symbol.upper()  # 统一转大写，例如 'HIVE'
...
if __name__ == "__main__":
    ...
    for coin in tqdm(common_coins):
        try:
            gate_io = Gateigate_io(coin)

            funding_rate, funding_interval = gate_io.get_futures_contract_info()  # 期货合约的 funding rate 与 interest rate
            interest_rate = gate_io.get_interest_rate()
            spot_quote_volume = gate_io.get_spot_quote_volume()  # 现货交易对的 quote volume
            future_quote_volume = gate_io.get_future_quote_volume()  # 期货交易对的 quote volume
            borrowable = gate_io.get_borrowable()  # 获取可借资产数据
            best_bid_spot, best_ask_spot = gate_io.get_spot_orderbook()  # 现货订单簿的最佳买卖价
            best_bid_future, best_ask_future = gate_io.get_futures_orderbook()  # 期货订单簿的最佳买卖价
            ...
            data_dict = {
                ...
                "spot_quote_volume": float(spot_quote_volume),
                "future_quote_volume": float(future_quote_volume),
    ...
    # Adjust funding_rate to a 2-hour basis when funding_interval > 2.0
    df.loc[df['funding_interval'] > 2.0, 'funding_rate'] = (
        df.loc[df['funding_interval'] > 2.0, 'funding_rate'] /
        (df.loc[df['funding_interval'] > 2.0, 'funding_interval'] / 2)
    )
    ...
    df['reverse_side_profit'] = df['reverse_side_spread'] - df['funding_rate'] - (df['interest_rate'] * 2)

    df['borrow_value'] = df['Borrowable'] * ((df['spot_best_bid'] + df['spot_best_ask']) / 2)
    
    # 按照正向套利收益从大到小排序
    df = df.sort_values("funding_rate", ascending=True) 

    df = df[(df['spot_quote_volume'] >= 100000) & (df['future_quote_volume'] >= 100000)]

    output_dir = '/data/market_maker/Gate_io/'
    os.makedirs(output_dir, exist_ok=True)

    csv_path = os.path.join(output_dir, "all_coins_USDT.csv")
    ...
    print(df[df["funding_rate"] > 0].sort_values("forward_side_profit", ascending=False).head(40))

    print('================================')  

    print(df[df["funding_rate"] < 0].sort_values("reverse_side_profit", ascending=False).head(40))
    print(sum(df[df["funding_rate"] < 0].sort_values("reverse_side_profit", ascending=False)['borrow_value'].head(29)))
