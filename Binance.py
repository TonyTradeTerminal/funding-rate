...
if __name__ == "__main__":
    ...
    for coin in tqdm(common_coins):
        try:
            ...
            data.append({
                ...
                'max_borrowable': float(max_borrowable),
    ...
    df['borrow_value'] = df['max_borrowable'] * ((df['spot_best_bid'] + df['spot_best_ask']) / 2).round().fillna(0)

    df = df.sort_values("funding_rate", ascending=True)
    df = df[(df['spot_24h_volume'] >= 100000) & (df['futures_24h_volume'] >= 100000)]


    df = df[['funding_rate', 'funding_rate_8h','interest_rate_8h', 'spot_24h_volume', 'futures_24h_volume', 'spot_best_bid','spot_best_ask','future_best_bid','future_best_ask', 'forward_side_spread', 'reverse_side_spread', 'forward_side_profit', 'reverse_side_profit', 'borrow_value']]
